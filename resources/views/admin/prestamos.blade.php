...
<center>
  <font color="orange" face="arial black">
    <h2>Procesando el prestamo del libro</h2>
  </font>
</center>
...
<!--  <font color="white" size="10">@{{saludo}}</font> -->
  ...
  <div class="container">
    ...
    <div class="row">
      <div class="col-xs-6">
        <div >          
          <input type="text" class="form-control" v-model="codigo" ref="buscar" v-on:keyup.enter="getLibro()" placeholder="Ingrese el codigo del libro">
          <br>
          <button class="btn btn-danger" @click="getLibro()">
            Agregar
          </button>
        </div>
        <br>
        <button class="btn btn-primary form-control" @click="prestar()">
          GUARDAR PRESTAMO
        </button>
      </div>
      <div class="col-xs-4">
        <a href="{{url('devoluciones')}}">
          <button class="btn btn-success glyphicon glyphicon-list" style="float: right;"> Verificar Prestamos</button>
        </a>
      </div>
    </div>
    <hr>
